  ...
  return vi.fn(async (input: string | URL | Request) => {
    ...
    if (url.endsWith("/metadata/react-state/0.2.0.json")) {
      return jsonResponse({
        ...
        version: "0.2.0",
        source_url: "https://gitlab.local/root/react-state",
        source_commit: "abc123",
        hash_sha256: "hash-1",
        license: "MIT",
        risk_level: "low",
        published_at: "2026-03-06T03:00:00.000Z"
      });
    }
    if (url.endsWith("/metadata/react-state/0.1.0.json")) {
      return jsonResponse({
        ...
        version: "0.1.0",
        source_url: "https://gitlab.local/root/react-state",
        source_commit: "abc122",
        hash_sha256: "hash-0",
        license: "MIT",
        risk_level: "low",
        published_at: "2026-03-05T03:00:00.000Z"
      });
    }
    if (url.endsWith("/metadata/design-system/1.0.0.json")) {
      return jsonResponse({
        ...
        version: "1.0.0",
        source_url: "https://gitlab.local/root/design-system",
        source_commit: "def123",
        hash_sha256: "hash-2",
        license: "Apache-2.0",
        risk_level: "medium",
    ...
    if (url.endsWith("/metadata/ops-skill/0.1.0.json")) {
      return jsonResponse({
        skill_id: "ops-skill",
        version: "0.1.0",
        source_url: "https://gitlab.local/root/ops-skill",
        source_commit: "ops001",
        hash_sha256: "hash-3",
        license: "Apache-2.0",
        risk_level: "high",
    ...
    if (url.includes("/install-manifests/")) {
      ...
      return jsonResponse({
        skill_id: skillId,
        version,
        package_url: `${rawBaseUrl}/packages/${skillId}/${version}.tgz`,
        package_sha256: `sha-${skillId}-${version}`,
        signature: "sig",
        public_key_id: "market-ed25519-v1",
        source_url: `https://gitlab.local/root/${skillId}`,
        published_at: "2026-03-06T03:00:00.000Z"
      });
    }
    ...
    if (url.endsWith("/attestations/react-state/0.2.0.json")) {
      return jsonResponse({
        ...
        approval: {
          ...
          decision: "approve",
          note: "clean review",
          reviewed_at: "2026-03-06T03:10:00.000Z"
        },
        signature: "sig",
    ...
    if (url.endsWith("/attestations/react-state/0.1.0.json")) {
      return jsonResponse({
        ...
        approval: {
          ...
          decision: "approve",
          note: "stable",
          reviewed_at: "2026-03-05T03:10:00.000Z"
        },
        signature: "sig",
        key_id: "market-ed25519-v1"
      });
    ...
    if (url.endsWith("/attestations/design-system/1.0.0.json")) {
      return jsonResponse({
        ingest_id: "ing-design",
        scan_issues: [{ rule: "network", severity: "medium", message: "network access", file: "SKILL.md" }],
        sandbox_result: { ran: true, runtime: "docker", ok: true, output: "ok" },
        approval: {
          ingest_id: "ing-design",
          reviewer: "bob",
          decision: "approve",
          note: "needs awareness",
    ...
    if (url.endsWith("/attestations/ops-skill/0.1.0.json")) {
      return jsonResponse({
        ingest_id: "ing-ops",
        scan_issues: [{ rule: "shell", severity: "high", message: "shell exec", file: "run.sh" }],
        sandbox_result: { ran: true, runtime: "docker", ok: false, output: "denied" },
        approval: {
          ingest_id: "ing-ops",
          reviewer: "carol",
          decision: "approve",
          note: "restricted ops",
          reviewed_at: "2026-02-15T10:00:00.000Z"
        },
        signature: "sig",
        key_id: "market-ed25519-v1"
      });
...
describe("gitlab catalog extras", () => {
  it("builds leaderboard modes from gitlab-backed releases", async () => {
    const service = new GitLabCatalogService({
      rawBaseUrl,
      fetchBaseUrl: rawBaseUrl,
      fetchImpl: createFetchFixture() as typeof fetch
    });

    const leaderboard = await service.getLeaderboard();

    expect(leaderboard.all_time[0]).toEqual(
  ...
  it("builds audits and categories with derived statuses", async () => {
    ...
    expect(audits).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          static_scan_status: "issues_detected",
    ...
    expect(designCategory).toEqual(
      expect.objectContaining({
        slug: "design",
        items: [expect.objectContaining({ skill_id: "design-system", category: "design" })]
      })
    );
  });
});
